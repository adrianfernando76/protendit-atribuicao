...
export default function App() {
  ...
  useEffect(() => { sched() }, [sel, openCts, vinculos, contratos, pessoas])
  ...
  function pLinks(pid) {
    return vinculos.filter(v=>v.pessoa_id===pid).map(v=>{
      for (const ct of contratos) {
        ...
        if(pc) return {label:`${ct.etapa} / ${pc.nome}`, vid:v.id, ctId:ct.id}
      }
      return null
    }).filter(Boolean)
  }
  ...
  function clickPessoa(pid) {
    ...
    const ids=[...new Set(vinculos.filter(v=>v.pessoa_id===pid).map(v=>{
      for(const ct of contratos){if(ct.pecas.find(p=>p.id===v.peca_id))return ct.id}
      return null
    }).filter(Boolean))]
    setOpenCts(ids)
  ...
  async function toggleImp(id,e) {
    e.stopPropagation()
    const ct=contratos.find(c=>c.id===id); if(!ct) return
  ...
  async function exportPDF() {
    ...
    Object.keys(G).forEach(grp=>{
      const lista=pessoas.filter(p=>p.grupo===grp); if(!lista.length) return
  ...
  const dash={
    ...
    porGrupo:Object.keys(G).map(g=>({label:G[g].label,cor:G[g].cor,count:pessoas.filter(p=>p.grupo===g).length,vc:vinculos.filter(v=>pessoas.find(p=>p.id===v.pessoa_id&&p.grupo===g)).length})),
  ...
  const ctsF=contratos.filter(ct=>(!filterImp||ct.importante)&&(!filterCt||ct.num.toLowerCase().includes(filterCt.toLowerCase())||ct.etapa.toLowerCase().includes(filterCt.toLowerCase())))
  ...
  const btnB={...btn,border:`1px solid ${B.mid}`,background:`${B.mid}15`,color:B.mid}
  const btnG={...btn,border:`1px solid ${B.green}`,background:`${B.green}15`,color:B.green}
  const btnA=a=>({...btn,border:`1px solid ${a?B.mid:B.border}`,background:a?`${B.mid}15`:'transparent',color:a?B.mid:B.muted})
  ...
  const modal={position:'fixed',inset:0,background:'rgba(0,0,0,.5)',zIndex:200,display:'flex',alignItems:'center',justifyContent:'center'}
  const mbox={background:'#fff',border:`1px solid ${B.border}`,borderRadius:12,padding:22,width:360,boxShadow:'0 8px 32px rgba(0,0,0,.15)',maxHeight:'80vh',overflowY:'auto'}
    ...
    <div style={{background:B.bg,width:'100vw',height:'100vh',fontFamily:"'Segoe UI',sans-serif",fontSize:13,color:B.text,display:'flex',flexDirection:'column',overflow:'hidden',position:'fixed',top:0,left:0}}>
      ...
      <div ref={boardRef} style={{flex:1,display:'grid',gridTemplateColumns:'1fr 1fr 1fr 3fr',overflow:'hidden',position:'relative'}}>
        ...
        <svg style={{position:'absolute',top:0,left:0,width:'100%',height:'100%',pointerEvents:'none',zIndex:50,overflow:'visible'}}>
          ...
          {arrows.map((a,i)=>{const mx=(a.x1+a.x2)/2;return<path key={i} d={`M${a.x1} ${a.y1} C${mx} ${a.y1} ${mx} ${a.y2} ${a.x2} ${a.y2}`} fill="none" stroke={B.teal} strokeWidth="1.5" opacity=".7" markerEnd="url(#arr)"/>})}
        </svg>
        ...
        {['projinternos','verificadores','projexternos'].map(grp=>{
          ...
          return (
            <div key={grp} style={{borderRight:`1px solid ${B.border}`,overflowY:'auto',display:'flex',flexDirection:'column',background:grp==='projinternos'?'#eef4ff':grp==='verificadores'?'#e6faf8':'#edfaee'}}>
              {/* Header coluna */}
              <div style={{padding:'6px 8px',borderBottom:`1px solid ${B.border}`,background:'rgba(255,255,255,.7)',position:'sticky',top:0,zIndex:10}}>
                ...
                {lista.map(p=>{
                  ...
                  return (
                    <div key={p.id} id={'p-'+p.id}
                      ...
                      {isSel&&links.length>0&&<div style={{display:'flex',flexDirection:'column',gap:2,marginTop:5}}>
                        {links.map((l,i)=>(
        ...
        <div style={{background:'#f5fbf8',borderLeft:`2px solid ${B.border}`,overflowY:'auto',padding:'6px'}}>
          <div style={{fontSize:10,fontWeight:700,color:B.teal,textTransform:'uppercase',letterSpacing:'.06em',marginBottom:6,padding:'2px 4px',position:'sticky',top:0,background:'#f5fbf8',zIndex:10}}>
            Contratos / Etapas — {ctsF.length} {filterImp?'importantes':'total'}
          </div>
            ...
            {ctsF.map(ct=>{
              ...
              return (
                <div key={ct.id} style={{background:'#fff',border:`1px solid ${isVinc?B.teal:ct.importante?B.amber:B.border}`,borderRadius:9,overflow:'hidden',boxShadow:isVinc?`0 0 0 2px ${B.teal}33`:'0 1px 3px rgba(0,0,0,.06)',transition:'all .2s'}}>
                  {/* Header */}
                  <div style={{padding:'6px 8px',cursor:'pointer',background:temV?`${B.teal}08`:'#fff',borderBottom:isOpen?`1px solid ${B.border}`:'none',display:'flex',flexDirection:'column',gap:2}}
                    onClick={()=>clickCt(ct.id)}>
                    <div style={{display:'flex',alignItems:'center',gap:4}}>
                      <span style={{fontSize:10,color:B.dim,transform:isOpen?'rotate(90deg)':'none',display:'inline-block',transition:'transform .2s'}}>▶</span>
                      <div style={{flex:1,minWidth:0}}>
                        <div style={{fontWeight:700,fontSize:12,color:B.text,whiteSpace:'nowrap',overflow:'hidden',textOverflow:'ellipsis'}}>{ct.etapa.toUpperCase()}</div>
                        <div style={{fontSize:10,color:B.mid,fontWeight:600}}>{ct.num.toUpperCase().startsWith('CT')?ct.num.toUpperCase():'CT '+ct.num.toUpperCase()}</div>
                      </div>
                      <div style={{display:'flex',gap:2,flexShrink:0}}>
                        {temV&&<span style={{fontSize:9,padding:'1px 4px',borderRadius:99,background:`${B.teal}20`,color:B.teal,fontWeight:700}}>{ct.pecas.filter(pc=>vinculos.some(v=>v.peca_id===pc.id)).length}</span>}
                        <span style={{color:ct.importante?B.amber:B.dim,fontSize:12,cursor:'pointer'}} onClick={e=>toggleImp(ct.id,e)}>{ct.importante?'★':'☆'}</span>
                        ...
                        <span style={{color:B.muted,fontSize:13,cursor:'pointer'}} onClick={e=>{e.stopPropagation();delCt(ct.id)}}>×</span>
                      </div>
                    </div>
                    {/* Iniciais fechado */}
                    {!isOpen&&<div style={{display:'flex',gap:2,flexWrap:'wrap',paddingLeft:14}}>
                      {ct.pecas.map(pc=>{
                        const linked=vinculos.some(v=>v.peca_id===pc.id)
                        const linkedSel=sel&&vinculos.some(v=>v.peca_id===pc.id&&v.pessoa_id===sel)
                        return <span key={pc.id} style={{fontSize:9,padding:'1px 3px',borderRadius:3,background:linkedSel?`${B.teal}30`:linked?`${B.teal}15`:'#f0f0f0',color:linkedSel?B.teal:linked?B.teal:B.dim,fontWeight:700}}>{pc.nome.slice(0,3).toUpperCase()}</span>
                      })}
                    </div>}
                  </div>
                  {/* Peças expandido */}
                  {isOpen&&ct.pecas.map(pc=>{
                    ...
                    return (
                      <div key={pc.id} id={'pec-'+pc.id}
                        style={{padding:'5px 8px 5px 20px',borderBottom:`1px solid ${B.border}`,cursor:sel?'pointer':'default',background:linkedSel?`${B.teal}12`:linked?`${B.teal}05`:'transparent',color:linkedSel?B.teal:linked?B.teal:B.muted,display:'flex',alignItems:'center',gap:6,fontSize:11,transition:'background .1s'}}
      ...
      {/* DASHBOARD */}
      {showDash&&<div style={{position:'fixed',inset:0,background:'rgba(0,0,0,.5)',zIndex:200,display:'flex',alignItems:'center',justifyContent:'center'}} onClick={()=>setShowDash(false)}>
      ...
      {/* MODAIS */}
      {modal&&<div style={{position:'fixed',inset:0,background:'rgba(0,0,0,.5)',zIndex:200,display:'flex',alignItems:'center',justifyContent:'center'}} onClick={()=>setModal(null)}>
